...
rbacRouter.post("/addPermissionToRole", async (req: Request, res: Response) => {
  try {
    ...
    if (!permissionRoleBody.roleId || !permissionRoleBody.permissionId) {
      res
        .status(400)
        .json("É necessário enviar o id da role e o id da permission!");
      return;
    }

    const permission = await permissionRepository.findOneBy({
      id: permissionRoleBody.permissionId,
    });
    ...
    if (role.permissions.find((x) => x.id == permission.id)) {
      res.status(400).json("A role já possui esta informação!");
      return;
    }
...
rbacRouter.post("/addRoleToUser", async (req: Request, res: Response) => {
  try {
    ...
    if (!userRoleBody.userId || !userRoleBody.roleId) {
      res.status(400).json("O id da role e o id do usuário sao obrigatórios!");
      return;
    }
    ...
    if (user.roles.find((x) => x.id == role.id)) {
      res.status(400).json("O usuário já possui esta role!");
      return;
    }
